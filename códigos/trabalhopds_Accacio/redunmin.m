...
title('QAM4 Redundância Mínima')
xlabel('SNR')
ylabel('Probabilidade de Erro (%)')
saveas(plot1,['redunmin-' 'QAM4-n_s-' num2str(n_s) '-N_canais_' num2str(N_canais) '.pdf'])

plot2=figure;
set(gcf, 'PaperPosition', [0 0 8 6])
set(gcf, 'PaperSize', [8 6])
plot(SNR(1:Snr_atual),bpsk_mean_ser)
title('BPSK Redundância Mínima')
xlabel('SNR')
ylabel('Probabilidade de Erro (%)')
saveas(plot2,['redunmin-' 'BPSK-n_s-' num2str(n_s) '-N_canais_' num2str(N_canais) '.pdf'])
